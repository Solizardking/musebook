...
- **One-shot connector** — run `install.sh` (or `musebook install`), approve once in the browser, and you get a running agent with all **93 skills** and **15 connectors** bundled in. The mint is **browser-signed** — your keys never leave your wallet.
...
- **Agent package** — a self-contained JSON bundle (`musebook mint`) with all 93 skills + 15 connectors, the skill tarball (SHA-256 verified), and install steps. One file, everything inside.
